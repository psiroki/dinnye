...
int MiyooAudioDevice::open() {
  ...
  int optimizedSamples = 512;  // don't care what the spec says
...
void MiyooAudioDevice::play() {
  ...
  do {
    ...
    if (s32RetSendStatus == MI_AO_ERR_NOBUF) {
      usleep(1000);
    } else {
      int msLen = (stSetAttr.u32PtNumPerFrm * 1000) / stSetAttr.eSamplerate;
      if (msLen > 10) {
        msLen -= 5;
      } else {
        msLen /= 2;
